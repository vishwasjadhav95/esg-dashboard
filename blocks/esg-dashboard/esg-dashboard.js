...
export default function decorate(block) {
  ...
  function setupDashboard() {
    ...
    // Listen for analysis complete event from analysis block
  ...
  async function generateQuickAnalysis() {
    ...
    try {
      // Use sample of countries/parameters for quick analysis
      ...
      // Show error message for quick analysis failure
  ...
  function verifyCountryCodes(countries) {
    ...
    countries.forEach(country => {
      if (knownValidCodes.includes(country.code)) {
        ...
        invalidCodes.push(country);
        // Include it anyway, API will handle the error
        validCodes.push(country);
      }
    });
    
    console.log(`🌍 Country verification: ${validCodes.length} valid, ${invalidCodes.length} potentially invalid`);
    return validCodes;
  }
  ...
  async function generateRealESGData(selections) {
    ...
    // Map parameter names to World Bank indicator codes with improved matching
    ...
    for (const param of selections.parameters) {
      ...
      if (esgIndicators[param]) {
        ...
        const indicator = Object.keys(esgIndicators).find(key => 
          param.toLowerCase().includes(key.toLowerCase().split(' ')[0]) ||
    ...
    console.log(`📊 Final indicators to fetch: ${selectedIndicators.length}/${selections.parameters.length}`);
    ...
    try {
      ...
      for (const country of validCountries.slice(0, 5)) { // Limit to 5 countries for performance
            ...
            if (response.ok) {
              ...
              if (result && result[1] && result[1].length > 0) {
                ...
                if (latestData) {
                  ...
                  // Store trend data
                  if (!data.countries[countryCode].trends[indicator]) {
                    data.countries[countryCode].trends[indicator] = [];
                  }
                  
                  result[1].forEach(yearData => {
                    if (yearData.value !== null) {
                      data.countries[countryCode].trends[indicator].push({
                        year: yearData.date,
  ...
  function createPerformanceChart(data, selections) {
    ...
    }).filter(dataset => dataset !== null); // Remove countries without real data
  ...
  function createParameterChart(data, selections) {
    ...
    const datasets = selections.countries.slice(0, 3).map((country, index) => {
      ...
      return {
        label: country.name || country.code,
        data: labels.map(() => {
          // Use real overall score for all parameters (simplified approach)
          return countryData.overall.score;
        }),
        backgroundColor: [
          'rgba(59, 130, 246, 0.8)',
          'rgba(139, 92, 246, 0.8)',
  ...
  function createTrendChart(data, selections) {
    ...
    const datasets = selections.countries.slice(0, 4).map((country, index) => {
      ...
      return {
        label: country.name || country.code,
        data: years.map(() => {
          // Use real overall score for trend (simplified - real implementation would use actual yearly data)
          return countryData.overall.score;
        }),
        borderColor: [
          'rgba(59, 130, 246, 1)',
  ...
  function handleAnalysisComplete(event) {
    ...
    // Already handled by button clicks above
  }
  
  // Test function to verify filter integration - call from browser console
  window.testESGFilters = function() {
