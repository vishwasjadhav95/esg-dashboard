// ESG Map Block - Content Driven
export default function decorate(block) {
  console.log('🗺️ ESG Map - Fixed based on working implementation');
  ...
  [...block.children].forEach((row) => {
    ...
    if (cells.length >= 5) {
      ...
      const displayName = getText(cells[2]);
      const lat = parseFloat(getText(cells[3]));
      const lng = parseFloat(getText(cells[4]));
      
      if (name && code && !isNaN(lat) && !isNaN(lng) && name !== '---') {
        countryData[name] = {
          code: code,
          name: displayName,
          coords: [lat, lng]
        };
        console.log(`✅ Added country: ${displayName} [${lat}, ${lng}]`);
      }
    }
  });
  
  // Replace table with map interface
  block.innerHTML = `
    <div class="map-header">
      <h2>🌍 Interactive Country Selection</h2>
    </div>
    
    <div class="map-interface">
      <div class="map-info">
        <div>
          <h3>Click on countries to select for analysis</h3>
          <p>Select multiple countries to compare ESG performance</p>
        </div>
  ...
  // Initialize the map (based on working implementation)
  ...
  // Load country boundaries using markers (based on working implementation)
  ...
  // Toggle country selection (based on working implementation)
  ...
  console.log(`✅ ESG Map: ${Object.keys(countryData).length} countries processed`);
} 